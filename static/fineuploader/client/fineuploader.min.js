...
qq.DragAndDrop=function(a){function b(){j===l&&!g&&(e.callbacks.log("Grabbed "+h.length+" files after tree traversal."),f.dropDisabled(!1),e.callbacks.dropProcessing(!1,h))}function c(a){var e;j+=1;a.isFile?a.file(function(a){h.push(a);l+=1;b()}):a.isDirectory&&(g=!0,a=a.createReader(),a.readEntries(function(a){l+=1;for(e=0;e<a.length;e+=1)c(a[e]);g=!1;a.length||b()}))}function d(a){f=new qq.UploadDropZone({element:a,onEnter:function(b){qq(a).addClass(e.classes.dropActive);b.stopPropagation()},onLeaveNotDescendants:function(){qq(a).removeClass(e.classes.dropActive)},
onDrop:function(d){e.hideDropzones&&qq(a).hide();qq(a).removeClass(e.classes.dropActive);d=d.dataTransfer;var g,k;e.callbacks.dropProcessing(!0);f.dropDisabled(!0);if(1<d.files.length&&!e.multiple)e.callbacks.error("tooManyFilesError","");else if(h=[],l=j=0,qq.isFolderDropSupported(d)){g=d.items;for(d=0;d<g.length;d+=1)if(k=g[d].webkitGetAsEntry())k.isFile?(h.push(g[d].getAsFile()),d===g.length-1&&b()):c(k)}else e.callbacks.dropProcessing(!1,d.files),f.dropDisabled(!1)}});k.addDisposer(function(){f.dispose()});
e.hideDropzones&&qq(a).hide()}var e,f,g,h=[],j=0,l=0,k=new qq.DisposeSupport;e={dropArea:null,extraDropzones:[],hideDropzones:!0,multiple:!0,classes:{dropActive:null},callbacks:{dropProcessing:function(){},error:function(){},log:function(){}}};qq.extend(e,a);return{setup:function(){e.dropArea&&e.extraDropzones.push(e.dropArea);var a,b=e.extraDropzones;for(a=0;a<b.length;a+=1)d(b[a]);e.dropArea&&(!qq.ie()||qq.ie10())&&k.attach(document,"dragenter",function(c){var d;if(d=!f.dropDisabled()){var g;qq.each(c.dataTransfer.types,
function(a,b){if("Files"===b)return g=!0,!1});d=g}if(d&&!qq(e.dropArea).hasClass(e.classes.dropDisabled)){e.dropArea.style.display="block";for(a=0;a<b.length;a+=1)b[a].style.display="block"}});k.attach(document,"dragleave",function(c){if(e.hideDropzones&&qq.FineUploader.prototype._leaving_document_out(c))for(a=0;a<b.length;a+=1)qq(b[a]).hide()});k.attach(document,"drop",function(c){if(e.hideDropzones)for(a=0;a<b.length;a+=1)qq(b[a]).hide();c.preventDefault()})},setupExtraDropzone:function(a){e.extraDropzones.push(a);
d(a)},removeExtraDropzone:function(a){var b,c=e.extraDropzones;for(b in c)if(c[b]===a)return c.splice(b,1)},dispose:function(){k.dispose();f.dispose()}}};
